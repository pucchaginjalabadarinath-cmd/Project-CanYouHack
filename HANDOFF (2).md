...
- Added `assignments.is_draft` (Boolean, default False) to `models.py` while
  building `routers/assignments.py` (ISSUES.md #6) — its acceptance
  criteria requires `GET /assignments` to hide "draft" assignments from
  students, and no such column existed yet. Defaults to published (False)
  since there's no dedicated publish/unpublish issue yet; professor can
  pass `is_draft=true` on create to hold one back.
